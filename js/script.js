$(document).ready(function () {
  ...
  $(".call-btn").on("click", function () {
    ...
    if ($btn.hasClass("arrived")) {
      console.log(`Elevator already at floor ${floor}. No action taken.`);
      return;
    }

    const elevatorAtFloor = elevators.find(
      (elevator) =>
        elevator.floor === floor && !elevator.moving && !elevator.busy
    );

    if (elevatorAtFloor) {
      console.log(
        `Elevator ${elevatorAtFloor.id} is already at floor ${floor}. No action needed.`
      );
      return;
    }
  ...
  function findClosestElevator(targetFloor) {
    ...
    console.log(
      `Closest elevator to floor ${targetFloor} is ${closestElevator ? closestElevator.id : "none"
      }`
    );
  ...
  function moveToFloor(elevator, targetFloor) {
    ...
    const targetBottom = targetFloor * 55;
    const elevatorLeftPosition = `${$elevator.position().left + 5}`;

    let $travelTimeDiv = $(`#travel-time-${targetFloor}`);
    if ($travelTimeDiv.length === 0) {
      $travelTimeDiv = $('<div>')
        .attr('id', `travel-time-${targetFloor}`)
        .addClass('travel-time')
        .css({
          display: 'flex',
          ...
          left: `${elevatorLeftPosition}px`,
          marginBottom: '12px',
        })
        .appendTo('#elevators');
    }
    // Start updating travel time
    const interval = setInterval(() => {
      const elapsed = Math.floor((new Date() - elevator.startTime) / 1000);
      const remainingTime = Math.max(0, elevator.travelTime / 1000 - elapsed);
      $travelTimeDiv.text(`Time Taken: ${remainingTime.toFixed(1)}s`);
      if (remainingTime <= 0) {
        clearInterval(interval);
        $travelTimeDiv.remove();
      }
    }, 0);

    // Move the elevator
    $elevator.css("transition", `bottom ${elevator.travelTime / 1000}s linear`);
    $elevator.addClass("moving").css("bottom", `${targetBottom}px`);
  ...
  function cancelElevatorCall(floor) {
    ...
    if (waitingTimes[floor]) {
      ...
      const $travelTimeDiv = $(`#travel-time-${floor}`);
      if ($travelTimeDiv.length > 0) {
        $travelTimeDiv.remove(); // Remove the travel time display
      }
